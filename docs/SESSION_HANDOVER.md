...
| Production                  | `4cf097ad5620`, read back from `GET https://verify.itisyou.app/health` at 15:31Z on 21 Sept |
| Staging                     | `4cf097ad5620`, deployed 15:30Z from the same gate artefact, one minute before production |
| Undeployed work             | None. Evidence: `docs/evidence/production-4cf097ad5620.txt` (the visual pass) and `docs/evidence/production-6fae121148ff.txt` (the three customer actions) |
...
| R16 | Owner panel walkthrough on production                                                                     | owner + lead | none (R17 is done)                         | Owner signs in at `https://verify.itisyou.app/admin/login` and confirms a TOTP code; the lead then reads `/owner`, `/owner/customers`, the billing and approval controls and the alert log, and records which are implemented and which are placeholders. **A production session must not be seeded and authentication must not be bypassed**, so this cannot start without the owner |
| R18 | **Compare the remaining eleven screens with their references**                                            | lead         | none                                       | `docs/screen-checklist.md` lists them: how-it-works, demo, support, the development story, five customer screens, five owner screens. They are rendered and captured at three widths but have not been read beside their reference. Render a reference with `reports/tmp/shoot-refs.mjs`, ours with the screens dump, and write the differences down |
...
| `curl -s https://verify.itisyou.app/health`                                                                                                            | commit `4cf097ad5620` (15:31Z, 21 Sept)                                                                  |
| `curl -s https://verify-itisyou-staging.kpleelaaravind.workers.dev/health`                                                                              | commit `4cf097ad5620` (15:30Z, 21 Sept)                                                                  |
