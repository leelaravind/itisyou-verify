...
| O13 | **Enrolment done 11:09:28Z; code confirmation still pending.** Your first code was refused with "reload the page and try again" — a CSRF defect in the enrolment response, fixed and deployed at 11:45Z (`b553e71`). Open `https://verify.itisyou.app/admin/authenticator` fresh → "Confirm a code now" → a current 6-digit code.                                                  | Every consequential action, including O14.                     |
