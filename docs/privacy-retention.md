...
| Account records                | Your email address, workspace, membership and workflow configuration.                                                                   | For as long as the account exists   | Removed when you delete the workspace                                                                           |
...
| Delete your workspace and its data                     | From the account page while signed in, or by sending us a support message | We schedule the deletion with a short grace period so it can be undone, then revoke sessions and stored credentials, stop scheduled checks, and remove evidence, runs, configuration, support messages and notification records. You get a statement of exactly what remains and why. |
...
When you delete a workspace we revoke your sessions and the provider credentials you gave
us, stop scheduled checks, expire shareable links, and remove your evidence, runs,
workflow configuration, support messages and notification records.
