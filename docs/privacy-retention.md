...
**Closed since the last audit — an entry point now exists and is proven.** This section's
own rule is that a row moves here only when the *trigger* is named, not when the code
exists. Each of these is driven by a test that starts at a request or a tick and finishes
by reading the database back.

| Promise                                                                                                               | What reaches it now                                                                                                                                                                                                                                                                                                                                                              |
| --------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Support bodies being stripped of credentials, tokens, card-shaped numbers and other addresses **before** storage (§2) | `D1CustomerDataPort.submitSupportRequest` → `recordSupportCase` → `createCase`, which redacts before it writes. The signed-in `/app/support` route is mounted on that port. Proven at the port (`CUST-300`–`CUST-302`) and through an HTTP request on the signed-out route (`API-432`), reading `support_cases.body_redacted` back.                                               |
| Support messages about deletion, billing disputes or security going straight to a person (§5)                         | The same call path. Triage sets the category, priority and starting state from the redacted text; nothing is hard-coded any more. `CUST-310`–`CUST-315`, and `API-433`/`API-434` through the route.                                                                                                                                                                              |
| The data export in §5                                                                                                 | `requestWorkspaceExport` in `apps/app/src/privacy/requests.ts` → `buildExport` → a link whose expiry comes from `exportLinkExpiry` → the `data_export_ready` email. The link is redeemed by `redeemExportLink`, which rebuilds the export for the workspace recorded against the token, so no second copy of your data is ever stored. `API-401`–`API-404`, `API-421`–`API-423`. |
| Workspace deletion in §5 and §6                                                                                       | `requestWorkspaceDeletion` schedules it and sends `deletion_scheduled`; `runDueWorkspaceDeletions` carries it out once the grace period has passed and sends `deletion_completed` — **only when every step finished**, never after a partial purge. `cancelWorkspaceDeletion` stops it. `API-411`–`API-416`, read back from `workspaces`, `memberships` and `workflows`.          |

**Still implemented, tested, and reached by nothing.** Each of these is written, has
passing tests, and has no route or tick that invokes it. None of it is a false description
of the code; all of it is a promise a customer cannot currently exercise:
...
| The four rows above reaching a **deployed** customer                             | The triggers and the signed-out router exist and are proven in integration, but `apps/app/src/index.ts` is the lead's file and the mount lines are not in it yet: `createPublicSupportRoute` is not routed, and no page posts to `requestWorkspaceExport` or `requestWorkspaceDeletion`. Until that lands, these are reachable in tests only. |
| A signed-out visitor being able to reach support (§5)                            | Same mount. `createPublicSupportRoute` answers `GET /support/contact` and `POST /support` and is proven by `API-431`–`API-437`; the public support page still publishes no address, because the owner has not supplied one — see `TODO_OWNER_INPUT` in §1.                                                                                    |
| `deletion_completed` firing from the production cron                             | `runDueWorkspaceDeletions` is written to be called from the tick. `apps/app/src/scheduler/tick.ts` has a different owner, so the call is a request rather than an edit.                                                                                                                                                                      |

Nothing in the tables above is a reason to soften the promises themselves. They are the
right promises; the remaining ones are simply not yet ones this service can keep, which is
precisely why it is not open. **This section comes out when each row has an entry point
named in it, not when the code exists.**
