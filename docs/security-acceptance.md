...
- **Gate A — COMMERCE.** Everything that must hold before a real card is charged, a real
  credential is stored, or a real customer's data enters the system. **12 of 21 rows are
  not yet passing.** Do not take money.
...
```
npx vitest run tests/security          → 221 cases: 217 pass, 4 FAIL   (all 4 are findings, below)
npx tsc -p tsconfig.json --noEmit      → 1 error, in apps/app/src/routes/owner/index.ts (A07, in flight)
                                          0 errors in tests/security
node scripts/scan-secrets.mjs --history → clean. 362 tracked files, full history.
```
...
| --- | --- | --- | --- |
| **A-01** | An unknown webhook endpoint id cannot be used to inject a forged provider event. | `SEC-431` | **FAIL — CRITICAL.** See finding F13. Exploit reproduced: status 200. |
| **A-02** | No cryptographic constant used in a comparison is a literal in this public repository. | `SEC-432` | **FAIL.** `DECOY_SECRET` is published. |
...
| **A-05** | An unknown endpoint and a wrong secret are indistinguishable to the caller. | `SEC-433` | **PASS.** (A06 built this deliberately; A-01 is the hole in it, not the absence of it.) |
...
| **A-14** | The same, judged on the **predicate** rather than the column list. | `SEC-206` | **FAIL.** 4 statements pass A-13 only because `workspace_id` is in their SELECT list. All 4 are correct cross-tenant sweeps; each needs its own marker. Finding F14. |
...
**Gate A verdict: DO NOT TAKE MONEY.** A-01 alone is disqualifying, and it is a
twenty-minute fix. A-20 and A-21 are real work.
...
| --- | --- | --- | --- | --- | --- |
| **F13** | **Any unknown webhook endpoint id accepts a forged event.** The route falls back to `DECOY_SECRET` when `resolveEndpointSecret` returns null and then acts on the result if the signature verifies. `DECOY_SECRET` is a literal in a **public repository**, so anyone can sign a body with it, POST to `/api/v1/webhooks/stripe/<invented-id>`, and have a fabricated `checkout.session.completed` or `invoice.paid` dispatched to the real handler. Reproduced: **expected 400, got 200.** | **CRITICAL** | A06 | `SEC-431` | **No** — the route is not yet mounted in `index.ts`. Fix **before** mounting. |
| **F14** | 4 statements satisfy the tenant-scope check only because `workspace_id` appears in their SELECT column list, not their predicate. All 4 are correct cross-tenant sweeps (retention paging, the due-run query); none is declared. | **Medium** | A02/A09 | `SEC-206` | n/a |
| **F15** | `packages/ui` interpolates `href` with no scheme guard, so `javascript:` survives intact. Mitigated twice today (all hrefs are literals; the CSP blocks it) and live the moment a link target comes from a CRM value, a report link or a campaign destination. | **Medium** | A05 | `SEC-1214` | Latent |

### The fix for F13, written out

```ts
const known = await deps.resolveEndpointSecret(opaqueId);
const verified = await verify(raw, signature, known ?? DECOY_SECRET, Date.parse(deps.now()));
if (known === null || !verified.valid) { /* the existing 400 */ }
```

Verification still runs for an unknown id, so `SEC-433` (indistinguishability) keeps
passing. Then derive the decoy from a Worker secret — an HMAC of the opaque id under
`SESSION_SIGNING_KEY` — so it is per-deployment and unguessable, closing `SEC-432`.
...
| **2026-09-19 (pm)** | **A10 pass 2** | **1 error (A07, in flight; 0 in `tests/security`)** | **221: 217 pass, 4 fail** | **clean, 362 files** | **FAIL** (A-01, A-02, A-14, A-20, A-21) | **PASS** |
| | | | | | | |

**The four failures, in full:**

```
SEC-431   unknown webhook endpoint accepts a forged event      F13, CRITICAL, A06
SEC-432   DECOY_SECRET is a literal in a public repository     F13, A06
SEC-206   4 statements scoped by column list, not predicate    F14, A02/A09
SEC-1214  javascript: survives into an href                    F15, A05
```

**Do not make these green by weakening a test.** Each is fixed by changing the code it
points at, or — for the four cross-tenant sweeps in `SEC-206` — by adding a
`tenant-scope:exempt <reason>` comment next to each statement, which records the decision
