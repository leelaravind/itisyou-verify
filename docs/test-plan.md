...
counts when it is _both_ passing _and_ countable — see §3.
...
### Step 2 — what makes a case _countable_
...
### Step 3 — what makes a case _passing_
...
filing artefact. Contrast, focus, keyboard operation and colour-independence _are_ tested —
...
`CUST-092`–`CUST-094` were previously passing _vacuously_ — they were measuring a 500 error
...
1. **`SEC-*` — 156 cases, 14 files, off-allowlist.** The single largest item. These are
   ...
   legitimate; the two documents must agree. Until then the gate is being met _without_
...
1. **The harvester redacts on the way in.** `api_key: '<REDACTED-FIXTURE>'` and
   ...
   _is_, which reads better than the original.
...
> `assigned-secret-literal` — _exactly_ the two that matched the fixtures in question. It
...
exists. The suite proves our adapters handle the payload _we believe_ the provider sends.
...
of those cases were passing _vacuously_ until A05 looked — measuring an error page.
...
| **Authenticated browser journey unmeasured** | 15 failing e2e cases; signed-in UI unverified at every breakpoint.  | Unit and integration cover the logic beneath. This is a gap in _interface_ evidence. Blocking for a UI release; A02's scoped identity closes it.                       |
...
provider _test_ account. They never write, never send. Running them also requires the lead
