...
| Production-origin checkout → activation → allowance → replay → customer access | **done 21 Sept 07:34–07:57Z.** Order `ord_01M31E6KYED181BAC8307E46E9` created by production, paid with Stripe's test card by the owner, three signed events `processed`, subscription `active`, one entitlement 500/0/0, resend answered `duplicate: true` with the row unchanged, `/app/billing` active and `/app/usage` 0/500 in the customer session | production DB + owner's browser |
