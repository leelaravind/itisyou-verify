...
| **A verified run does not prove the acknowledgement reached the enquirer** | **OPEN — release-relevant.** The CRM half is sound: the record is retrieved *by* the run's correlation value via `searchContactByCorrelation`, so the per-run binding happens at retrieval and `exists` on it is a real assertion. The email half is not: the event is retrieved by message id, and the compiler then asserts only that `message.recipient` `exists`. That proves an email had *a* recipient, not that it went to the person who enquired. `normalised_email_equals` is implemented in the evaluator and emitted by nothing except the demo port, so the demo shows a check a real workflow never performs — and the development story documents a careful safeguard (not stripping `+tags`, because `boss+anything@corp.com` must not satisfy a rule about `boss@corp.com`) on a comparison that does not run | Either bind at retrieval as the CRM side does, or let the evaluator take a per-run expected value. Until then, qualify the demo and the story | verification and connectors + lead | — | A run where the acknowledgement went to the wrong address comes back FAILED, not VERIFIED |
