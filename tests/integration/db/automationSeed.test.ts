...
describe('the automation seed', () => {
  ...
  it('AUTH-440 emits the cookie name that matches the origin, which A07 must not assume', () => {
    const local = buildAutomationSeed({ environment: 'development', baseUrl: HTTP_ORIGIN, now: NOW });
  ...
  it('AUTH-447 the shell script derives the same names and the same ceiling as the module', () => {
...
describe('the approval claim store', () => {
  ...
  it('OWNER-320 the store runs the statement A07 exported, not a second spelling of it', async () => {
  ...
  it('OWNER-321 spends a granted approval exactly once', async () => {
    grant('apr_1');
  ...
  it('OWNER-322 two concurrent claims of one approval: exactly one holds the permission', async () => {
    grant('apr_1');
    const store = createApprovalClaims(h.db);
  ...
  it('OWNER-323 refuses a revoked, an expired and an absent approval alike', async () => {
  ...
  it('OWNER-324 an approval expiring exactly now is refused, not accepted', async () => {
