...
test.describe('customer journey', () => {
  ...
  test('CUST-081 the workspace shows the verification rate and the activity signal as two separate readouts', async ({ page }) => {
    await page.goto('/app');
    await expect(page.getByRole('heading', { name: 'Verification rate' })).toBeVisible();
    await expect(page.getByRole('heading', { name: 'Are enquiries still arriving?' })).toBeVisible();
  ...
  test('CUST-082 the run detail shows expected against observed with the recipient masked', async ({ page }) => {
  ...
  test('CUST-083 an unverified run is explained as "we could not look", never as a failure', async ({ page }) => {
  ...
  test('CUST-084 the run list pages by cursor, and the newer control is disabled on the first page', async ({ page }) => {
  ...
  test('CUST-085 a form submitted with an invalid value re-renders the error beside its own field', async ({ page }) => {
  ...
  test('CUST-086 the whole mapping step can be completed from the keyboard alone', async ({ page }) => {
    await page.goto('/app/onboarding/mapping');
  ...
  test('CUST-088 refusing every required check is rejected with a reason, not silently accepted', async ({ page }) => {
    await page.goto('/app/onboarding/outcome');
    for (const name of [
    ...
    const message = page.getByText('With nothing required, a verified result would not mean anything.');
    await expect(message.first()).toBeVisible();
    expect(await message.count()).toBeGreaterThanOrEqual(1);
  });

  test('CUST-089 the checkout hand-off never claims a payment it did not take', async ({ page }) => {
  ...
  test('CUST-090 the support form reports what actually happened to the message', async ({ page }) => {
    await page.goto('/app/support');
    await page.locator('#f-subject').fill('A run showed as unverified');
    await page.locator('#f-body').fill('Run run_syn_0004 says unverified and I want to understand why.');
  ...
  test('CUST-091 cancellation says plainly that there is nothing to cancel rather than offering a dead button', async ({ page }) => {
