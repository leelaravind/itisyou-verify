...
describe('applyLateEvidence', () => {
  ...
  it('VERIFY-126 late evidence cannot be applied to a run that has not been decided', () => {
    ...
    expect(() => applyLateEvidence(run, originalResults, { now: T_AFTER_DEADLINE })).toThrowError(AppError);
  });
  ...
  it('VERIFY-129 the original revision is preserved untouched, including its status and timing', () => {
    ...
    applyLateEvidence(run, [mandatoryResult('email_delivered', 'SUPPORTED', 'MATCHED')], { now: T_AFTER_DEADLINE });
  ...
  it('VERIFY-130 the new revision references the same rule set the original was judged against', () => {
    const run = makeRunRevision({ status: 'UNVERIFIED', rules_ref: 'wf-1@v7', rules_schema_version: 1 });
  ...
  it('VERIFY-131 looking late is recorded as a late observation, not as a late completion', () => {
    ...
    const onTime = [makeResult({ rule_id: 'rule_1', status: 'SUPPORTED', observed_at: isoAfter(T_EVENT, 120) })];
  ...
  it('VERIFY-132 a provider event whose own timestamp missed the deadline is a late completion', () => {
    const run = makeRunRevision({ status: 'UNVERIFIED' });
    const lateWork = [makeResult({ rule_id: 'rule_1', status: 'SUPPORTED', observed_at: isoAfter(T_EVENT, 900) })];
  ...
  it('VERIFY-134 late evidence that contradicts a previously verified run produces a FAILED revision', () => {
    const run = makeRunRevision({ status: 'VERIFIED', results: [mandatoryResult('rule_1', 'SUPPORTED', 'MATCHED')] });
    const outcome = applyLateEvidence(run, [mandatoryResult('rule_1', 'CONTRADICTED', 'VALUE_MISMATCH')], {
      now: T_AFTER_DEADLINE,
    });
  ...
  it('VERIFY-135 revisions chain: a second late arrival supersedes the first revision, not the original', () => {
    ...
    const second = applyLateEvidence(first.revision!, [mandatoryResult('rule_1', 'CONTRADICTED', 'VALUE_MISMATCH')], {
  ...
  it('VERIFY-136 a same-status revision with different reasons is still recorded as a new revision', () => {
    ...
    const outcome = applyLateEvidence(run, [mandatoryResult('rule_1', 'UNKNOWN', 'RECORD_AMBIGUOUS')], {
      now: T_AFTER_DEADLINE,
    });
    expect(outcome.changed).toBe(true);
  ...
  it('VERIFY-137 detectLateCompletion ignores results that carry no timestamp', () => {
    ...
    expect(detectLateCompletion([makeResult({ observed_at: isoAfter(T_EVENT, 601) })], T_DEADLINE)).toBe(true);
    expect(detectLateCompletion([makeResult({ observed_at: isoAfter(T_EVENT, 600) })], T_DEADLINE)).toBe(false);
  });
});
