...
const REASON_TEXT: Record<ReasonCode, { sentence: string; next_step: string | null }> = {
  MATCHED: {
    sentence: 'We retrieved this from the connected system and it matches what you asked us to check.',
    next_step: null,
  },
  VALUE_MISMATCH: {
    sentence: 'We retrieved this from the connected system and the value is not the one your rule expects.',
    next_step: 'Compare the expected and observed values below; if the rule is the thing that is out of date, edit the check.',
  },
  RECORD_NOT_FOUND: {
    sentence: 'We searched the connected CRM and it has no record carrying this enquiry reference.',
    next_step: 'Check that your automation writes the correlation reference onto the record it creates.',
  },
  RECORD_AMBIGUOUS: {
    sentence:
  ...
  RECORD_WRONG_ACCOUNT: {
    sentence: 'The record we were pointed at belongs to a different provider account from the one connected here.',
    next_step: 'Connect the account that actually holds these records, or point the automation at the connected account.',
  },
  EVENT_NOT_OBSERVED: {
    sentence: 'We asked the email provider for events on this message and it reported none.',
    next_step: 'Check that your automation actually sent the acknowledgement, and that it used the connected sending account.',
  },
  STATUS_NOT_REACHED: {
    sentence:
    ...
    next_step: 'If this keeps happening for one recipient domain, that domain is likely rejecting or delaying your mail.',
  },
  OUTSIDE_TIME_WINDOW: {
    sentence: 'We found the evidence, but its own timestamp falls outside the window your rule allows.',
    next_step: 'Widen the window if the automation is meant to take longer, or look at what is delaying it.',
  },
  EVIDENCE_UNAVAILABLE: {
    sentence: 'We could not retrieve the evidence this check needs, so it is unverified rather than failed.',
  ...
  EVIDENCE_NOT_RETURNED: {
    ...
    next_step: 'Check that the connected account is allowed to read this field, and that the field is actually in use.',
  },
  ...
  CONNECTION_UNAVAILABLE: {
    ...
    next_step: 'Reconnect the provider from the Connections page if it is showing as expired or missing permission.',
  },
  DEADLINE_PASSED: {
    sentence: 'The agreed completion window closed before we could confirm this check.',
    next_step: 'If your automation legitimately takes longer than this, raise the completion window for this workflow.',
  },
  AWAITING_EVIDENCE: {
  ...
  CORRELATION_MISSING: {
    sentence: 'The record we found does not carry the enquiry reference we use to tie it to this enquiry.',
    next_step: 'Set the correlation property on the record your automation creates so we can match it with confidence.',
  },
  RULE_UNSUPPORTED: {
    sentence: 'This check is configured in a way we cannot evaluate, so we have not judged it either way.',
...
const RUN_TEXT: Record<RunStatus, { headline: string; sentence: string; next_step: string | null }> = {
  VERIFIED: {
    headline: 'Verified',
  ...
  FAILED: {
    ...
    next_step: 'Open the checks below to see exactly which one, what we expected and what we found.',
  },
  UNVERIFIED: {
    headline: 'Unverified',
    sentence:
    ...
    next_step: 'Check the connection status for the provider named below; if it is healthy, no action is needed.',
  },
  PENDING: {
    headline: 'Still checking',
    sentence: 'The agreed completion window is still open, so we are continuing to look for evidence.',
    next_step: null,
  },
};
